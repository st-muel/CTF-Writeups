...
We could use either `LibreOffice` or upload the file to Google Docs to view it, but `LibreOffice` doesn't come with Kali so we would need to install it. Since I'm too lazy let's use this command from StackOverflow instead to simply extract all the text from the document! (https://stackoverflow.com/a/25620447)
